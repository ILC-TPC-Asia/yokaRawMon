...
void ResidualbyZ(Int_t b = 1, Int_t layer = 17, Double_t p = 5.0)
{
//--Set Style
  // Axis
...
//                        { 18564, 18563, 18562, 18561, 18552, 18551, 18550, 18549, 18548, 18547}}; // B=1T
  ...
  titlestr << "Residual by Z (Row" << layer << ")" << ends;
...
void Process(Int_t run, Int_t layer, Double_t &x, Double_t &y, Double_t &dx, Double_t &dy, const Char_t *inout, Double_t p)
{
  ...
  y  = htemp->GetFunction("gaus")->GetParameter(1); // sigma
  dx = 0;
  dy = htemp->GetFunction("gaus")->GetParError(1);  // its error
  x  *= 10.; // [cm] to [mm]
  y  *= 10.; // [cm] to [mm]
  dx *= 10.; // [cm] to [mm]
  dy *= 10.; // [cm] to [mm]
