...
void GMResol_Angle(Int_t b = 1, Int_t layer = 14, Double_t p = 5.0)
{
//--Set Style
  // Axis
  ...
  Int_t kRun[2][kNp] = {{  19717,19743,19746},  // B=0T
...
//                        { 18609, 18606, 18605, 18601, 18600, 18598, 18597, 18595, 18594, 18592, 18591}}; // B=1T
  ...
  // ------------------------
    cerr << "Fill z v.s. sigma_x plot:start" << endl;
  ...
  for (Int_t i=0; i<kNp; i++) {
    ...
    cerr << "Fill z v.s. sigma_x plot:end" << endl;
  ...
  grp->GetHistogram()->GetXaxis()->SetLimits(0.,25.);
  grp->GetHistogram()->GetXaxis()->SetTitle("Angle [degree]");
  ...
  titlestr << "GM Resolutin by Angle (Row" << layer << ")" << ends;
...
void Process(Int_t run, Int_t layer, Double_t &x, Double_t &y, Double_t &dx, Double_t &dy, const Char_t *inout, Double_t p)
{
...
#ifdef CHECK_DIST
  stringstream hstr;
  hstr << "c" << "_" << rinfo.GetAngle(run) << "_" << inout << ends;
  ...
  stringstream finstr;
  cerr << "run=" << run << ends;
  finstr << "10k/p." << p << "_z." << rinfo.GetDlength(run) << "_B." << rinfo.GetBfield(run) << "_φ." << rinfo.GetAngle(run) << ".root" << ends;
  ...
  TNtupleD *hResXin = static_cast<TNtupleD *>(hfp->Get("hResXin"));
  stringstream item;
  item << "dx" << inout << setw(2) << setfill('0') << layer << ends;
  stringstream cut;
  cerr << "hResXin entries = " << hResXin -> GetEntries() << endl;
  ...
  cut << item.str().data() << ">" << kResXmin << "&&"
      ...
      << "ndf>"  << kNdfCut    << "&&"
      << "chi2<" << kChi2Cut   << "&&"
      << "((fi0" << layer << ">"<< kPhi0MinCut << "&&"
      << "fi0"   << layer << "<"  << kPhi0MaxCut << ")||"
      << "(fi0"  << layer << "+2*TMath::Pi()>"  << kPhi0MinCut << "&&"
      << "fi0"   << layer << "+2*TMath::Pi()<"  << kPhi0MaxCut << "))&&"
      << "tnl>"  << kTanlMinCut << "&&"
      << "tnl<"  << kTanlMaxCut << "&&"
